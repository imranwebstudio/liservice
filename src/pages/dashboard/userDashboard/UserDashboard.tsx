...
/* ── types ─────────────────────────────────────────────── */
type View = 'overview' | 'services' | 'orders' | 'recharge' | 'addBalance';
...
const UserDashboard = () => {
  ...
  const { data: ordersData }    = useGetPendingServiceByUserIdQuery(undefined);
  const { data: balanceData }   = useGetBalanceByUserIdQuery(undefined);
  ...
  return (
    <div className={`db-root${theme === 'light' ? ' light' : ''} min-h-screen flex bg-[var(--db-bg0)] text-[var(--db-t0)]`}>
      ...
      {/* ── Sidebar ─────────────────────────────────── */}
      <aside className={`db-sidebar${sideOpen ? ' open' : ''} fixed inset-y-0 left-0 z-40 w-[260px] flex flex-col border-r border-[var(--db-line)]`} style={{ background: 'linear-gradient(180deg,var(--db-bg1) 0%,var(--db-bg0) 100%)' }}>
        {/* logo */}
        <div className="flex items-center gap-2.5 px-6 h-[70px] border-b border-[var(--db-line)] flex-shrink-0">
        ...
        {/* profile card */}
        <div className="mx-4 mt-4 mb-2 px-4 py-3.5 rounded-[16px] border border-[var(--db-line)] bg-[rgba(255,255,255,0.025)]">
          <div className="flex items-center gap-3">
            {user?.photo ? (
              <img src={user.photo} alt={user.name} className="w-10 h-10 rounded-full object-cover border border-[var(--db-g5)]" />
            ) : (
              <span className="w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm text-[#06150d] flex-shrink-0"
            ...
            <div className="min-w-0">
              ...
              <div className="text-[11.5px] text-[var(--db-t2)] truncate">{user?.email ?? ''}</div>
            </div>
          </div>
          <div className="mt-3 flex items-center justify-between pt-2.5 border-t border-[var(--db-line)]">
            <span className="text-[11.5px] text-[var(--db-t2)]">Balance</span>
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-2">
          <div className="text-[10.5px] font-bold tracking-[0.12em] uppercase text-[var(--db-t2)] px-3 mb-2 mt-2">Main Menu</div>
          {NAV_ITEMS.map(({ id, label, Icon }) => (
            <button key={id}
              onClick={() => navigate(id)}
              className={`db-nav-btn relative w-full flex items-center gap-3 px-3.5 py-3 rounded-[12px] mb-0.5 text-[14px] font-medium transition-[background,color] cursor-pointer ${view === id ? 'active text-[var(--db-g3)] bg-[rgba(52,217,126,0.1)]' : 'text-[var(--db-t1)] hover:bg-white/[0.045] hover:text-[var(--db-t0)]'}`}>
              <span className={`flex-shrink-0 transition-colors ${view === id ? 'text-[var(--db-g3)]' : 'text-[var(--db-t2)]'}`}><Icon /></span>
              {label}
            </button>
          ))}

          <div className="text-[10.5px] font-bold tracking-[0.12em] uppercase text-[var(--db-t2)] px-3 mb-2 mt-4">Quick Links</div>
          <a href="/" className="w-full flex items-center gap-3 px-3.5 py-3 rounded-[12px] mb-0.5 text-[14px] font-medium text-[var(--db-t1)] hover:bg-white/[0.045] hover:text-[var(--db-t0)] transition-colors">
            <svg className="text-[var(--db-t2)] flex-shrink-0" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
            Home
          </a>
        </nav>

        {/* bottom: theme + logout */}
        <div className="px-4 pb-5 pt-3 border-t border-[var(--db-line)] flex-shrink-0 space-y-2">
          <button onClick={() => setTheme(t => t === 'dark' ? 'light' : 'dark')}
            className="w-full flex items-center justify-between px-3.5 py-3 rounded-[12px] text-[14px] font-medium text-[var(--db-t1)] border border-[var(--db-line2)] hover:bg-white/[0.04] transition-colors cursor-pointer">
            <span>{theme === 'dark' ? 'Dark Mode' : 'Light Mode'}</span>
            <span className="w-9 h-5 rounded-full relative border border-[var(--db-line2)] transition-colors" style={{ background: theme === 'dark' ? '#1c4a2e' : 'rgba(255,255,255,0.08)' }}>
              <span className={`absolute top-[2px] w-[14px] h-[14px] rounded-full transition-[transform,background] ${theme === 'dark' ? 'translate-x-4 bg-[#34d97e]' : 'translate-x-[2px] bg-[var(--db-t2)]'}`} />
            </span>
          </button>
          <button onClick={() => dispatch(logout())}
            className="w-full flex items-center gap-3 px-3.5 py-3 rounded-[12px] text-[14px] font-medium text-[#f4677a] hover:bg-[rgba(244,103,122,0.08)] transition-colors cursor-pointer">
            <IcLogout />Logout
          </button>
        </div>
      </aside>
      ...
      <div className="relative z-10 flex-1 flex flex-col min-h-screen md:ml-[260px]">
        {/* top bar */}
        <header className="sticky top-0 z-20 flex items-center justify-between h-[70px] px-6 border-b border-[var(--db-line)] [background:rgba(var(--db-bg0-raw,6,13,10),0.82)] backdrop-blur-[14px]">
