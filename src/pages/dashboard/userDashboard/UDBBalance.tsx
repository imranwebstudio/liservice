...
import { useGetPaymentConfigQuery, useGetPaymentMethodsQuery } from '../../../redux/features/payment/paymentMethod.api';
import Loading from '../../../utils/Loading';

interface PayMethod { _id: string; name: string; color: string; iconUrl?: string; qrCode?: string; number: string; steps: string[]; }

function MethodIcon({ m, size = 24 }: { m: PayMethod; size?: number }) {
  const isUrl = m.iconUrl && (m.iconUrl.startsWith('http') || m.iconUrl.startsWith('/') || m.iconUrl.startsWith('data:'));
  if (isUrl) {
    return (
      <span className="rounded-md overflow-hidden flex items-center justify-center shrink-0"
        style={{ width: size, height: size, background: m.color }}>
        <img src={m.iconUrl} alt={m.name} style={{ width: size, height: size, objectFit: 'contain' }} />
      </span>
    );
  }
  return (
    <span className="rounded-md flex items-center justify-center text-[13px] font-bold text-white shrink-0"
      style={{ width: size, height: size, background: m.color }}>
...
const UDBBalance = ({ onSuccess }: Props) => {
  ...
  const { data: methodsData, isLoading: loadingMethods } = useGetPaymentMethodsQuery(undefined);
  const { data: configData,  isLoading: loadingConfig  } = useGetPaymentConfigQuery(undefined);
  ...
  const panelCls = "rounded-[20px] border border-(--db-line) p-6 db-panel [background:linear-gradient(165deg,rgba(255,255,255,0.038),rgba(255,255,255,0.01))]";
  const inputCls = "w-full h-13 px-4.5 rounded-[13px] border border-(--db-line2) bg-(--db-input) text-(--db-t0) text-[15px] outline-none transition-[border-color,box-shadow] focus:border-(--db-g5) focus:shadow-[0_0_0_4px_rgba(31,191,108,0.12)]";
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
        ...
        <div className="flex items-center gap-3.5 px-5 py-[18px] mb-4.5 rounded-[18px] border border-(--db-line) [background:linear-gradient(165deg,rgba(255,255,255,0.04),rgba(255,255,255,0.012))]">
          <div className="w-12 h-12 rounded-full bg-[#06120c] border border-(--db-line2) flex items-center justify-center">
            <span className="w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold text-[#06150d]"
              style={{ background: 'linear-gradient(155deg,#6ee7a8,#149656 60%,#2dd4cf)' }}>L</span>
          </div>
          <div>
            <div className="font-semibold text-(--db-t0)" style={{ fontFamily: "'Space Grotesk',sans-serif" }}>liservice24.com</div>
            <div className="text-[12.5px] text-(--db-t2)">Official payment portal</div>
          </div>
          <div className="ml-auto font-bold text-[26px] text-(--db-g4)" style={{ fontFamily: "'Space Grotesk',sans-serif" }}>
        ...
        {/* steps */}
        {active && (
          <div className="rounded-[18px] px-5.5 mb-4.5" style={{ border: `1px solid ${borderColor}`, background: bgColor }}>
            {steps.map((s, i) => (
              <div key={s.n} className={`flex gap-3 py-3.5 text-sm leading-[1.55] text-(--db-t1) ${i < steps.length - 1 ? 'border-b border-white/7' : ''}`}>
                <span className="w-6 h-6 rounded-full bg-white/8 flex items-center justify-center text-[12px] font-bold text-(--db-t0) shrink-0"
        ...
        {active?.qrCode && (
          <div className="mb-4.5 flex flex-col items-center gap-2">
            <p className="text-[13px] text-(--db-t2) self-start">Scan QR to pay</p>
            <img src={active.qrCode} alt={`${active.name} QR`}
        ...
        {/* verify button */}
        <button disabled={!canVerify || isLoading} onClick={submit}
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
      ...
      <div className={panelCls}>
        ...
        {/* fee */}
        <div className="mb-5">
          <label className={labelCls}>Extra Fee ({(FEE_RATE * 100).toFixed(2)}%)</label>
          <input className={`${inputCls} text-(--db-t2) cursor-not-allowed`} disabled
        ...
        <div className="mb-5">
          ...
          <input className={`${inputCls} text-(--db-g3) font-bold text-[18px] cursor-not-allowed`} disabled
        ...
        <div className={`db-check-row flex items-center gap-3 mb-6 cursor-pointer select-none ${termsOk ? 'checked' : ''}`}
          ...
          <span className="text-[13.5px] text-(--db-t1)">Accept the <a href="#" className="text-(--db-g3)" onClick={e => e.stopPropagation()}>terms and conditions</a></span>
        </div>

        <button disabled={!canPay} onClick={goToPayment}
      ...
      <div className={`${panelCls} sticky top-6`}>
        <h3 className="font-semibold text-[17px] text-(--db-t0) mb-1" style={{ fontFamily: "'Space Grotesk',sans-serif" }}>Order Summary</h3>
        <p className="text-[13px] text-(--db-t2) mb-3.5">Live breakdown of your top-up.</p>
        {[
          ['Dollar amount',                             `$${usd.toFixed(2)}`],
          ['Conversion rate',                           `৳ ${RATE.toFixed(2)} / $1`],
          [`Extra fee (${(FEE_RATE*100).toFixed(2)}%)`, `৳ ${feeTaka.toFixed(2)}`],
        ].map(([label, val]) => (
          <div key={label} className="flex items-center justify-between py-3 border-b border-(--db-line) text-sm text-(--db-t1)">
            <span>{label}</span>
            <b className="text-(--db-t0)" style={{ fontFamily: "'Space Grotesk',sans-serif" }}>{val}</b>
          </div>
        ))}
        ...
        <div className="flex gap-2.5 mt-4.5 p-3 rounded-xl bg-[rgba(45,212,207,0.06)] border border-[rgba(45,212,207,0.18)] text-[12.5px] text-(--db-t1) leading-normal">
          <svg className="text-(--db-teal) shrink-0 mt-0.5" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10" /><line x1="12" y1="16" x2="12" y2="12" /><line x1="12" y1="8" x2="12.01" y2="8" /></svg>
