  ...
  return (
    <nav
      style={{
        ...
        zIndex: 100,
        padding: '12px 0',
        background: scrolled || mobileOpen ? 'rgba(7,11,9,0.97)' : 'rgba(7,11,9,0.55)',
      ...
      <div className="max-w-295 mx-auto px-8 flex items-center justify-between">
        ...
        <div className="hidden md:flex gap-9">
          {NAV_LINKS.map(({ label, to }) => (
            <Link key={label} to={to} className="home-nav-link">{label}</Link>
          ))}
          {user && <Link to="/dashboard" className="home-nav-link">Dashboard</Link>}
        </div>
        ...
        <div className="hidden md:flex items-center gap-3.5">
          {user && (
            <div style={{
              ...
              border: '1px solid #28392d', borderRadius: 999,
              padding: '8px 16px', fontSize: 13.5, fontWeight: 500, color: '#aebcb2',
            }}>
        ...
        >
          <span className={`block w-5 h-0.5 bg-[#f3fbf5] rounded-sm origin-center transition-[transform,opacity] duration-300 ${mobileOpen ? 'translate-y-1.75 rotate-45' : ''}`} />
          <span className={`block w-5 h-0.5 bg-[#f3fbf5] rounded-sm origin-center transition-[transform,opacity] duration-300 ${mobileOpen ? 'opacity-0 scale-x-0' : ''}`} />
          <span className={`block w-5 h-0.5 bg-[#f3fbf5] rounded-sm origin-center transition-[transform,opacity] duration-300 ${mobileOpen ? '-translate-y-1.75 -rotate-45' : ''}`} />
        </button>
      </div>

      {/* Mobile drawer */}
      <div className={`overflow-hidden transition-[max-height] duration-350 ease-in-out ${mobileOpen ? 'max-h-125' : 'max-h-0'}`}>
        <div className="px-6 py-7 border-t border-[#1d2c23]">
          ...
          <div className="flex flex-col gap-1">
            {NAV_LINKS.map(({ label, to }) => (
              <Link
                key={label} to={to} onClick={closeMobile}
            ...
            {user && (
              <Link
                to="/dashboard" onClick={closeMobile}
                className="block py-3 px-3.5 text-[#aebcb2] no-underline text-base font-medium rounded-[10px] transition-[background,color] duration-200 hover:bg-[rgba(52,217,126,0.08)] hover:text-[#f3fbf5]"
              >
                Dashboard
              </Link>
          ...
          <div className="flex flex-col gap-3.5">
            {user && (
              <div className="flex justify-between items-center px-3.5 py-2.5 border border-[#1d2c23] rounded-[10px] text-sm">
                ...
                <span style={{ color: '#f3fbf5', fontWeight: 600 }}>$ {data?.data?.balance ?? '0.00'}</span>
              </div>
            )}
            {user
              ? <SubscribeButton label="Logout" onClick={handleLogout} />
